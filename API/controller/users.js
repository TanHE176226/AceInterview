...
//   return jwt.sign(userPayload, process.env.ACCESS_TOKEN_SECRET, { expiresIn: '1h' });
// };
...
const registerRecruiter = async (req, res) => {
  const { username, password, email, fullName, companiesID, Workplace } =
    req.body;
  try {
    // Kiểm tra xem email có tồn tại chưa
    const existingUser = await userDAO.findUserByEmail(email);
...
const login = async (req, res) => {
  ...
  try {
    ...
    const { token: refreshToken, expiresAt } = await createRefreshToken(user._id);
    // Set the access token in the response header
    res.set("Authorization", "Bearer " + accessToken);

    return res.status(200).
      json
      ({ success: true, user, accessToken, refreshToken, refreshTokenExpiresAt: expiresAt });
  } catch (error) {
    console.error(error);
...
const sendMail = async (req, res, next) => {
  ...
  try {
    ...
    console.log(userDAO.updatePassword(email, newPassword));
...
const sendMailRecruiter = async (req, res, next) => {
  ...
  try {
    ...
    const info = await transporter.sendMail({
      ...
      text: ` ${user.fullName} with ${cv.fileURL} applied for ${job.title}`,
    });
